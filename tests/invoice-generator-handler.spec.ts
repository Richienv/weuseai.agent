...
test('drift: agent-packs/doc-expert/templates/invoice-pro.html matches inlined constant', () => {
  // Per Phase 2E-1.5 pivot: template moved into the doc-expert bundle.
  // Edge Functions can't read from disk at runtime, so the handler ships
  // INVOICE_TEMPLATE_HTML inline; this test catches divergence between
  // the canonical .html file in the bundle and the inlined constant.
  const onDisk = readFileSync(
    resolve(REPO_ROOT, 'agent-packs/doc-expert/templates/invoice-pro.html'),
    'utf8',
  )
  assert.equal(
    onDisk,
    INVOICE_TEMPLATE_HTML,
    'invoice-pro.html on disk drifted from INVOICE_TEMPLATE_HTML constant in invoice-generator-handler.ts. Sync them.',
  )
})
