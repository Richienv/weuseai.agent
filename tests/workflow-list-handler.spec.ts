...
function row(overrides: Partial<WorkflowRow> = {}): WorkflowRow {
  return {
    ...
    agent_slugs: ['the-pro'],
    category: 'analysis',
    intent_phrases: ['phrase1', 'phrase2', 'phrase3', 'phrase4', 'phrase5'],
...
test('list: returns ok + array of public-shape items, no sensitive fields', async () => {
  ...
  assert.equal((item as Record<string, unknown>).intent_embedding, undefined)
  ...
  assert.equal((item as Record<string, unknown>).intent_phrases, undefined)
})

test('list: intent_phrases_sample limited to 3 entries', async () => {
  const r = await workflowListHandler({}, fakeReader([row()]))
  if (!r.ok) assert.fail('expected ok')
  assert.equal(r.workflows[0].intent_phrases_sample.length, 3)
  assert.deepEqual(
    r.workflows[0].intent_phrases_sample,
